...
public class P9019 {
	...
	public static void main(String[] args) {
		...
		for(int test=0; test<t; test++) {
			...
			String[] oper = new String[P9019.MAX];
			boolean[] check = new boolean[P9019.MAX];
			Queue<Integer> queue = new LinkedList<>();
			int[] from = new int[P9019.MAX];
...
//			oper[a] = "";
			...
			while(!queue.isEmpty()) {
				...
				// D 연산
				int dOperNum = nowNum * 2;
				if(dOperNum < P9019.MAX) {
					if(!check[dOperNum]) {
						queue.add(dOperNum);
						oper[dOperNum] = "D"; 
						check[dOperNum] = true;
						from[dOperNum] = nowNum;
					}
				}else {
					if(!check[dOperNum%10000]) {
						queue.add(dOperNum%10000);
						oper[dOperNum%10000] = "D"; 
						check[dOperNum%10000] = true;
						from[dOperNum%10000] = nowNum;
					}
				}
				
				// S 연산
				int sOperNum = nowNum - 1;
				if(nowNum == 0) {
					if(!check[9999]) {
						queue.add(9999);
						oper[9999] = "S"; 
					...
					if(!check[sOperNum]) {
						...
						oper[sOperNum] = "S"; 
						check[sOperNum] = true;
						from[sOperNum] = nowNum;
					}
				}
				
				// L 연산
				int lOperNum = nowNum%1000*10 + nowNum/1000;
				if(!check[lOperNum]) {
					queue.add(lOperNum);
					oper[lOperNum] = "L"; 
					check[lOperNum] = true;
					from[lOperNum] = nowNum;
				}
				
				// R 연산
				int rOperNum = nowNum%10*1000 + nowNum/10;
				if(!check[rOperNum]) {
					queue.add(rOperNum);
					oper[rOperNum] = "R"; 
					check[rOperNum] = true;
					from[rOperNum] = nowNum;
				}
			}
			
			Stack<String> operOrder = new Stack<>();
			operOrder.push(oper[b]);
			int prevNum = from[b];
			while(prevNum != a) {
				operOrder.push(oper[prevNum]);
				prevNum = from[prevNum];
			}
			
			while(!operOrder.isEmpty()) {
				System.out.print(operOrder.pop());
			}
			System.out.println();
		}
		scan.close();
